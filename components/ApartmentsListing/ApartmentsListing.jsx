...
import { APARTMENT_LISTINGS as LISTINGS } from "@/data/apartmentListings";

const TEAL = "#0d5c5c";
const TEAL_HOVER = "#0a4a4a";

/** Hero for /apartments — swap file in /public/images when you have the final asset */
const APARTMENTS_HERO_IMAGE = "/images/palmcentral.jpg";
...
function PropertyCard({ property }) {
  ...
  return (
    <article className="bg-white border border-slate-200 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow flex flex-col h-full">
      <div className="relative aspect-[4/3] bg-slate-100 group/slide">
        <Swiper
          modules={[Pagination]}
...
export default function ApartmentsListing() {
  ...
  const totalCount = 835;

  return (
    <main className="bg-white text-slate-900 pb-16">
      {/* Full-width hero — Our Properties › Residential › Apartments */}
      <section
        className="relative w-full min-h-[380px] md:min-h-[460px]"
        aria-labelledby="apartments-hero-heading"
      >
        <div className="absolute inset-0 z-0">
          <Image
            src={APARTMENTS_HERO_IMAGE}
            alt=""
            fill
            priority
            className="object-cover"
        ...
        <div className="relative z-10 mx-auto flex min-h-[380px] max-w-4xl flex-col items-center justify-center px-6 pb-14 pt-28 text-center md:min-h-[460px] md:pt-32 md:pb-20">
          <p className="mb-3 text-xs font-semibold uppercase tracking-[0.2em] text-white/80">
            Our Properties · Residential · Apartments
          </p>
          <h1
            id="apartments-hero-heading"
            className="text-3xl font-bold tracking-tight text-white md:text-5xl md:leading-tight"
          >
            Apartments in Dubai
          </h1>
          <p className="mt-4 max-w-2xl text-base text-white/90 md:text-lg">
            Browse off-plan and ready apartments across Dubai&apos;s most popular communities and
            developments.
          </p>
        </div>
      </section>
      ...
      <div className="mx-auto max-w-7xl px-4 pt-8 sm:px-6 lg:px-8">
        {/* Breadcrumb — Our Properties > Residential > Apartments */}
        <nav
        ...
        >
          ...
          <ChevronRight size={14} className="text-slate-300 shrink-0" />
          <span className="font-semibold text-slate-900">Apartments</span>
        </nav>
        ...
        {/* Results + sort — tight gap under filters like reference */}
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
          <p className="text-sm md:text-base text-slate-700">
            Off Plan Properties in Dubai{" "}
            <span className="text-slate-300">|</span>{" "}
            <span className="font-bold text-slate-900">{totalCount} properties</span>
          </p>
