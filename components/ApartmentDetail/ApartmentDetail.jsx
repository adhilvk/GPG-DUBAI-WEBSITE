...
import { getRelatedListings } from "@/data/apartmentListings";
...
function RelatedCard({ p }) {
  ...
  return (
    <article className="border border-slate-200 rounded-lg overflow-hidden bg-white shadow-sm flex flex-col">
      <Link href={`/apartments/${p.slug}`} className="relative aspect-[4/3] block bg-slate-100">
...
export default function ApartmentDetail({ property }) {
  ...
  const related = useMemo(() => getRelatedListings(property.slug, 3), [property.slug]);
  ...
  const listingUrl = `${siteBase}/apartments/${property.slug}`;
    ...
    <main className="bg-white text-slate-900 pb-20">
      ...
      <section className="bg-slate-900 pt-24 pb-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <Link href="/apartments" className="hover:text-white hover:underline">
              Back to search
            </Link>
            <span className="text-white/40">/</span>
            <span>Off plan</span>
            <span className="text-white/40">/</span>
            <span>Apartments in Dubai</span>
            <span className="text-white/40">/</span>
